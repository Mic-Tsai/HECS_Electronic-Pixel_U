...
#define N_LEDS       120 // Max value is 170 (fits one SD card block)
...
#define CURRENT_MAX 500 // Max current from power supply (mA)
...
void setup() {
  ...
  if(startupTrigger == LOW) { // No button press
...
void loop() {
  ...
  while(digitalRead(TRIGGER) == LOW);   // Wait for trigger button
  ...
  for(;;) {
    ...
    if(++block >= nBlocks) {                   // Past last block?
      if(digitalRead(TRIGGER) == LOW) {       // Trigger released?
